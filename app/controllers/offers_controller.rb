class OffersController < ApplicationController
    ...
    def create
        ...
        if req && req.user_id == current_user.id
            redirect_to request.referrer, alert: "Não pode responder aos teus proprios pedidos!"
        end

        if Offer.exists?(user_id: current_user.id, request_id: offer_params[:request_id])
            redirect_to request.referrer, alert: "Uma oferta de cada vez!"
        end

        @offer = current_user.offers.build(offer_params)
        if @offer.save
            # redirect_to request.referrer, notice: "Saved..."
            redirect_to my_offers_path, notice: "Gardado..."
        else
    ...
    def accept
        if @offer.pending?
            ...
            if charge(@offer.request, @offer)
                ...
                flash[:alert] = "Não foi possivel criar o pedido! "
            end
        end
    ...
    def is_authorised
        redirect_to root_path, alert: "Sem permissão" unless current_user.id == @offer.request.user_id
    end
